...
poipath = "/home/faizan/USA_data/h11v05/output_near/"

shapeData = ogr.Open(aoipath + "us_near_rivr.shp")
...
def EVI():
    ...
    for point in points:
        lat = point[1]
        lon = point[0]
        out_csv = poipath + "EVI.near%02d.csv" % (n)
        listData = []
        for j in images:
...
def LSWI():
    ...
    for point in points:
        lat = point[1]
        lon = point[0]
        out_csv = poipath + "LSWI.near%02d.csv" % (n)
        listData = []
        for j in images:
            if os.path.exists(j) is True:
                y = os.path.split(j)[1].split(".")[0]
...
def NDVI():
    ...
    for point in points:
        lat = point[1]
        lon = point[0]
        out_csv = poipath + "NDVI.near%02d.csv" % (n)
        listData = []
        for j in images:
            if os.path.exists(j) is True:
                y = os.path.split(j)[1].split(".")[0]
...
def FLOOD():
    ...
    for point in points:
        lat = point[1]
        lon = point[0]
        out_csv = poipath + "FLOOD.near%02d.csv" % (n)
        listData = []
        for j in images:
            if os.path.exists(j) is True:
                y = os.path.split(j)[1].split(".")[0]
