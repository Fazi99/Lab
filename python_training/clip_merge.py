...
"""
"""
limage = Index("/home/faizan/HDD1/anomaly/cru.2001.01.cld.anomaly.tif")
fill=-999
...
for typ in dtype:
    for year in years:
        for i in range(1, 13):
            full="/home/faizan/HDD1/anomaly/cru.%d.%02d.%s.anomaly.tif"  % (year, i, typ)
            clip="/home/faizan/HDD1/anomaly/cru_pak.%d.%02d.%s.anomaly.tif"  % (year, i, typ)
...
limage = Index("/home/faizan/HDD1/LSWI/2004/MODIS.LSWI.2004004.h23v06.tif")
fill=-999
years=[2004]
shape="/home/faizan/Pakistan/pakistan shape file/PAK_adm0.shp"
...
for year in years:
    for i in range(1, 363):
        ...
        for V in range(5,7):
            for H in range(23,25):
                files.append("/home/faizan/HDD1/LSWI/"+str(year)+"/MODIS.LSWI.%d%03d"  % (year, i)+ ".h%dv%02d" % (H,V) + ".tif" )
        merge="/home/faizan/HDD1/LSWI/"+str(year)+"/mosaic.%d%03d"  % (year, i) + ".tif"
        limage.MergeWithGdalwarp(files, merge)
        clip="/home/faizan/HDD1/LSWI/"+str(year)+"/clip.%d%03d"  % (year, i) + ".tif"
        limage.ClipWithGdalwarp(merge, clip, shape)
        for f in files:
           os.remove(f)
        os.remove(merge)
...
"""
limage = Index("/home/faizan/HDD1/EVI/2010/Pak_crop.2010022.tif")
#years=[2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014]
shape="/home/faizan/Desktop/shapefile/Pak_cropped_area.shp"
for n in range(2010,2011):
    year=n
    for i in range(1, 363):
        full="/home/faizan/HDD1/FLOOD/2010/flood.%d%03d.tif"  % (n, i)
        #clip="/home/faizan/Pakistan/EVI/"+str(year)+"/Pak_crop.%d%03d"  % (year, i) + ".tif"
        clip="/home/faizan/HDD1/FLOOD/2010/flood_crop.%d%03d.tif"  % (n, i)
        limage.ClipWithGdalwarp(full, clip, shape)
