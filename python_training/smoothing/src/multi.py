...
wavelet = pywt.Wavelet(pywt.waveflist()[16])
modis = MOD_Wavelet("/".join(cdir.split('/')[:-2]))


EVI_dir = "/".join(cdir.split('/')[:-2]) + "/EVI/"
numpy_dir = "/".join(cdir.split('/')[:-2]) + "/numpy/"
...
def myfunc(i):
    ...
    if sum(flag) > 20:
        ...
        evidenoise = numpy.array([-28672. for k in range(362)], dtype=numpy.int32)
...
if __name__ == '__main__':
    for y in range(2005, 2006):
        print y
        inputfiles = sorted(glob.glob(EVI_dir + "MOD09A1.A" + str(y) + "*.EVI.tif"))
        print inputfiles
