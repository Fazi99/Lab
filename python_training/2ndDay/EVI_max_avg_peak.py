...
def find_max_evi_date(mask, year, start, end):
    files = [path+str(year)+name+".%d%03d"  % (year, i) + ".tif" for i in range(start, end) ]
...
def find_avg_evi_K(year):
    files = [path+str(year)+name+".%d%03d"  % (year, i) + ".tif" for i in range(91,335) ]
    mimage = MultiImages(files)
    AllArrays = mimage.CalcAverage()
    return AllArrays

def find_max_evi_K(year):
    files = [path+str(year)+name+".%d%03d"  % (year, i) + ".tif" for i in range(91,335) ]
    mimage = MultiImages(files)
    AllArrays = mimage.SumAllArrays()
    return AllArrays

def find_avg_evi_R(year1, year2):
    file1 = [path+str(year1)+name+".%d%03d"  % (year1, i) + ".tif" for i in range(274, 363) ]
    file2 = [path+str(year2)+name+".%d%03d"  % (year2, j) + ".tif" for j in range(1, 91) ]
    files=file1+file2
    mimage = MultiImages(files)
    AllArrays = mimage.CalcAverage()
    return AllArrays
...
def find_max_evi_date_K(mask, year):
    files = [path+str(year)+name+".%d%03d"  % (year, i) + ".tif" for i in range(91,274) ]
    mimage = MultiImages(files)
    AllArrays = mimage.Images2Arrays()
    results = numpy.array([0 for i in AllArrays[0]])
    for i in numpy.where(mask.flatten() == 1)[0]:
...
path1="/home/faizan/HDD1/EVI/2002/Pak_crop.2002099.tif"
ds = gdal.Open(path1)
...
for year in range(2002, 2015):
    for i in range(len(days)-1):
        ...
        image = GImage(path1)
        m=mon[i]
        #image.Array2Tiff(out_name="/home/faizan/HDD1/EVI/%d/Pak_crop.%d.%s_max_evi_value.tif" % (year,year,m), Array=max_value)
        image.Array2Tiff(out_name="/home/faizan/Pakistan/EVI/%d/sindh.%d.%s_avg_evi_value.tif" % (year,year,m), Array=avg_value)
        #print m
...
for n in range(len(years)):
    image = GImage(path1)
    max_value = find_max_evi_K(years[n])
    image.Array2Tiff(out_name="/home/faizan/HDD1/EVI/EVI_max/Pak_crop.%d.kharif_sum.tif" % (years[n]), Array=max_value)
    del max_value

    avg_value = find_avg_evi_K(years[n])
    image.Array2Tiff(out_name="/home/faizan/HDD1/EVI/EVI_max/Pak_crop.%d.kharif_avg_evi_value.tif" % (years[n]), Array=avg_value)
    del avg_value
    print n
"""
    max_date = find_max_evi_date_K(r_array, years[n])
    image.Array2Tiff(out_name="/home/faizan/HDD1/EVI/%d/Pak_crop.%d.kharif_max_evi_date.tif" % (years[n],years[n]), Array=max_date)
    del max_date

    max_date = find_max_evi_date_R(r_array, years[n], years[n+1])
    image.Array2Tiff(out_name="/home/faizan/HDD1/EVI/%d/Pak_crop.%d.rabi_max_evi_date.tif" % (years[n],years[n]), Array=max_date)
    del max_date

    avg_value = find_avg_evi_R(years[n], years[n+1])
    image.Array2Tiff(out_name="/home/faizan/HDD1/EVI/%d/Pak_crop.%d.rabi_avg_evi_value.tif" % (years[n],years[n]), Array=avg_value)
    del avg_value
    print years[n]
"""
"""
...
def input_lists(month):
    ...
    return [path+str(year)+name+"."+str(year)+"."+month+"_avg_evi_value.tif"   for year in xrange(2002, 2014) ]
    #return ["tiff/MODIS.EVI.MonthlySum.%d%02d.tif" % (year, month) for year in xrange(2001, 2014)]

def output_lists(month):
    return [path+str(year)+name+"."+str(year)+"."+month+"_evi_anomaly.tif"   for year in xrange(2002, 2014) ]
